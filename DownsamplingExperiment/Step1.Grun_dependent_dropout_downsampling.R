...
save(all.samples, file = paste0("Grun_downsampling_rate", le))
...
save(all.samples, all.samples.before.dr, all.ori, file = paste0("Grun_downsampling_flag_rate", le))


s1 <- table(all.samples==0 & all.samples.before.dr ==0 & all.ori!=0)
s2 <- table(all.samples==0 & all.samples.before.dr !=0 & all.ori!=0)

save(s1, s2, file = paste0("Grun_downsampling_zero_summary_rate", le))
...
for(le in c(0.5, 0.6, 0.7, 0.8, 0.9, 0.95)){
	...
	save(gene.expression, file = paste0("Grun_filtered_rate", le))

	write.csv(gene.expression, paste0("Grun_filtered_rate", le, ".csv"))
	write.csv(t(gene.expression), paste0("Grun_filtered_rate", le, "_t.csv"))
...
system.time(scimpute(count_path = paste0("Grun_filtered_rate", le, ".csv"),
...
load(paste0("Grun_filtered_rate", le))
library(DrImpute)
...
save(lnpX_imp, file = paste0("Grun_drImpute_filtered_rate", le))
...
save(saver_imp, file = paste0("Grun_SAVER_filtered_rate", le))

###############################################  
############# running MAGIC ################### 
############################################### 

python MAGIC.py -d Grun_filtered_rate0.5_t.csv -o Grun_filtered_rate0.5_magic.csv --mols-per-cell-min 0 --mols-per-cell-max 100000000 csv
python MAGIC.py -d Grun_filtered_rate0.6_t.csv -o Grun_filtered_rate0.6_magic.csv --mols-per-cell-min 0 --mols-per-cell-max 100000000 csv
python MAGIC.py -d Grun_filtered_rate0.7_t.csv -o Grun_filtered_rate0.7_magic.csv --mols-per-cell-min 0 --mols-per-cell-max 100000000 csv
python MAGIC.py -d Grun_filtered_rate0.8_t.csv -o Grun_filtered_rate0.8_magic.csv --mols-per-cell-min 0 --mols-per-cell-max 100000000 csv
python MAGIC.py -d Grun_filtered_rate0.9_t.csv -o Grun_filtered_rate0.9_magic.csv --mols-per-cell-min 0 --mols-per-cell-max 100000000 csv
python MAGIC.py -d Grun_filtered_rate0.95_t.csv -o Grun_filtered_rate0.95_magic.csv --mols-per-cell-min 0 --mols-per-cell-max 100000000 csv
...
save(imp_ee, file = paste0("Grun_lasso_filtered_rate", le))
...
save(imp_el_ee, file = paste0("Grun_el_filtered_rate", le))
