 ...
 for(i in c(0.5, 0.6, 0.7, 0.8, 0.9, 0.95)){
	...
	scimpute <- read.csv(paste0("scImpute/Shalek_rate", i, "scimpute_count.csv"), as.is = T)
	scimpute.imp <- scimpute[, -1]

	load(paste0("Shalek_drImpute_res_rate", i))	
	lnpX.imp <- lnpX_imp
	
	load(paste0("Shalek_SAVER_res_rate", i))
	saver.imp <- saver_imp$estimate
	
	load(paste0("Shalek_lasso_filtered_rate", i))
	imputed.imp <- imp_ee$imputed 
	
	magic <- read.csv(paste0("Shalek_filtered_rate", i, "_magic.csv"))
	magic.imp <- t(magic[, -1])
	
	load(paste0("Shalek_filtered_rate", i))
	imputed.el.imp <- imp_el_ee$imputed	
	
	save(scimpute.imp, lnpX.imp, saver.imp, imputed.imp, magic.imp, imputed.el.imp, file = paste0("Shalek_impute_res_rate", i, "_imputation.summary"))
...
for(i in c(0.5, 0.6, 0.7, 0.8, 0.9, 0.95)){
	...
	scimpute <- read.csv(paste0("scImpute/Shalek_uniform_rate", i, "scimpute_count.csv"), as.is = T)
	scimpute.imp <- scimpute[, -1]

	load(paste0("Shalek_drImpute_uniform_res_rate", i))	
	lnpX.imp <- lnpX_imp
	
	load(paste0("Shalek_SAVER_uniform_res_rate", i))
	saver.imp <- saver_imp$estimate
	
	load(paste0("Shalek_lasso_filtered_uniform_rate", i))
	imputed.imp <- imp_ee$imputed 
	
	
	magic <- read.csv(paste0("Shalek_filtered_uniform_rate", i, "_magic.csv"))
	magic.imp <- t(magic[, -1])
	
	load(paste0("Shalek_el_filtered_uniform_rate", i))
	imputed.el.imp <- imp_el_ee$imputed	
	
	save(scimpute.imp, lnpX.imp, saver.imp, imputed.imp, magic.imp, imputed.el.imp, file = paste0("Shalek_impute_uniform_res_rate", i, "_imputation.summary"))
	
}



xx <- read.delim("GSE48968-GPL13112-count.txt")
pheno <- read.delim("GSE48968-GPL13112-phenotype.txt", header = F)
types <- unique(pheno[, 2])
...
for(i in c(0.5, 0.6, 0.7, 0.8, 0.9, 0.95)){
	...
	load(paste0("Shalek_flag_rate", i))
	...
	load(paste0("Shalek_impute_res_rate", i, "_imputation.summary"))
...
correlation.summary1 <- data.frame(dataset = rep("Shalek", nrow(all.res)), 
...
L1.summary1 <- data.frame(dataset = rep("Shalek", nrow(all.res)), 
           downsampling = c(0.5, 0.6, 0.7, 0.8, 0.9, 0.95),
           scimpute = all.res2[, 1],
           lnpX = all.res2[, 2], 
           saver = all.res2[, 3],
...
L2.summary1 <- data.frame(dataset = rep("Shalek", nrow(all.res)), 
           downsampling = c(0.5, 0.6, 0.7, 0.8, 0.9, 0.95),
           scimpute = all.res3[, 1],
           lnpX = all.res3[, 2], 
           saver = all.res3[, 3],
...
correlation.summary2 <- data.frame(dataset = rep("Shalek", nrow(all.res)), 
           downsampling = c(0.5, 0.6, 0.7, 0.8, 0.9, 0.95),
           scimpute = all.res[, 1],
           lnpX = all.res[, 2], 
           saver = all.res[, 3],
...
L1.summary2 <- data.frame(dataset = rep("Shalek", nrow(all.res)), 
           downsampling = c(0.5, 0.6, 0.7, 0.8, 0.9, 0.95),
           scimpute = all.res2[, 1],
           lnpX = all.res2[, 2], 
           saver = all.res2[, 3],
...
L2.summary2 <- data.frame(dataset = rep("Shalek", nrow(all.res)), 
           downsampling = c(0.5, 0.6, 0.7, 0.8, 0.9, 0.95),
           scimpute = all.res3[, 1],
           lnpX = all.res3[, 2], 
           saver = all.res3[, 3],
...
save(correlation.summary1, L1.summary1, L2.summary1, correlation.summary2, L1.summary2, L2.summary2, file = "Shalek_downsampling_comparison_summary_CorL1L2")
...
for(i in c(0.5, 0.6, 0.7, 0.8, 0.9, 0.95)){
	...
	load(paste0("Shalek_filtered_uniform_rate", i))
	sub.xx <- xx[rownames(gene.expression), colnames(gene.expression)]
	
	load(paste0("Shalek_uniform_flag_rate", i))
	...
	load(paste0("Shalek_impute_uniform_res_rate", i, "_imputation.summary"))
	log.scimpute.pred <- apply(scimpute.imp, 2, function(x){log(x+1)})
	log.lnpX.pred <- lnpX.imp
	log.imputed.pred <- apply(imputed.imp, 2, function(x){log(exp(x)-0.1+1)})
	log.magic.pred <- apply(magic.imp, 2, function(x){log(x+1)})
...
save(correlation.summary1, L1.summary1, L2.summary1, correlation.summary2, L1.summary2, L2.summary2, file = "Shalek_downsampling_uniform_comparison_summary_CorL1L2")
